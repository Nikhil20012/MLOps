...
### Task Update
- Replaced binary classification with multi-class classification across 7 forest cover types.

### Metrics Update
- Replaced AUC with Accuracy and Weighted F1 Score as evaluation metrics, which are more appropriate for multi-class classification.

### Model Update
- Kept Random Forest as the baseline model.
- Added XGBoost with automated hyperparameter tuning using Optuna across 5 trials.
...
2. Perform EDA, check for missing values, and plot class distribution
...
**Script runs slowly**
- The dataset has 581k rows. Random Forest takes around 2-3 minutes and XGBoost tuning takes around 5-8 minutes. This is expected.
...
- Make sure the script has completed successfully and the model is registered before running the serve command.
